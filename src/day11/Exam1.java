    ...
    public static void main(String[] args) {
        ...
        // [2] 예외종류
            ...
            // 2) 실행예외 : 실행 도중에 발생하는 예외 처리 코드 검사( 개발자 예측/경험 의존도 크다)
                // 연산문제, null, index등 주로 코드적인문제, 타입문제
        // [3] 목적 : 1) 예외 고치는게 아니라 예외가 발생하면 피해서 다른 코드로 이동, 즉] 24시간 코드 실행
        // [4] 사용법
            // try { 예외가 발생하거나 발생할 것 같은 코드 }
            // catch( 발생한예외클래스명 변수명 ) { 만약에 예외발생 했을때 처리할 코드 }
            // finally { 예외 여부 상관없이 무조건 싱행 되는 코드 }
        // 1. ClassNotFoundException : 클래스를 못 찾았을 때 발생하는 예외 클래스 ..일반예외
        // 2. InterruptedException : 흐름(스레드)가 중단 되었을 때 발생하는 예외 클래스 .. 일반예외
        // 3. NullPointerException : null 일때 (.)접근연산자 사용시 발생하는 예외 클래스 .. 실행예외
        // 4. NumberFormatException : 정수타입으로 변환이 불가능한 변수를 변환시킬 때 발생하는 예외 클래스  .. 실행예외
        ...
        try{
            Class.forName("java.lang.String2" ); // - String 클래스가 존재하는지확인
        ...
        try {
            ...
            int value = scan.nextInt(); //만약 비정상적인 입력값이면 **예외발생
        }catch ( InputMismatchException e){
            System.out.println("예외발생6 : 입력할 수 없는 정보 또는 잘못된 형식입니다." + e);
        }
        // 7) 다중 catch , 예외는 다양한 이유로 발생한다. 예외 중에 상위클래스가 존재한다. Exception 클래스
        try{
        ...
        }catch ( Exception e){ // 모든 예외에 대해 처리해주는 상위 클래스, 다중 catch할 경우 가장 마지막에 사용한다.
