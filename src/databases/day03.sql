...
# [1] INSERT : 레코드/행/튜플/1줄/*DT01개 삽입
# 1) insert into 테이블명( 속성명1 , 속ㅇ명2 ) values( 값1, 값2 );
...
# 5) not null 제약조건 속성은 꼭 삽입 자료가 존재해야 한다.alter
...
insert into test values( 6 , "하하" , 70 ) , ( 7 , "바나나" , 31 );

-- [2] select , 레코드 조회 , 주의할점 : 조회 결과테이블과 원본테이블 상관없다.
# 1) select * from 테이블명;
select * from test;
...
select mid from member as m;	-- member테이블명을 m으로 별칭한다.
