...
# 주의할점 : select * from buy group by bpname; 그룹 기준 외 다른 속성들의 대해 어떻게 보여줄지 정의 안됨.
...
select min( bamount ) from buy; -- buy 테이블내 bamount 속성값들의 전체 중 최소값
...
# 2) 그룹 후 조건 having 총구매금액(그룹후에 존재하는 속성)이 10000 이상인
...
select * from member order by mdebut desc;
	# -> 1차 정렬에서 maddr(주소) 정렬 한 후 동일한 maddr(주소) 끼리 mdebut(날짜) 2차정렬한다.
select * from member order by maddr desc , mdebut asc;

# [6] limit : 조회 결과 제한 , (검색/페이징)
select * from member limit 2; 		-- 조회 결과 위에서 2개만 조회
select * from member limit 0,2;  	-- 조회 결과 0(첫번째레코드) 부터2개 조회한다.
