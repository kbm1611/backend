...
create table test3(
    ...
    정수속성4 int , 정수속성5 bigint , 정수속성6 int unsigned , -- signed부호있는(+-) unsigned(없음)
    ...
    문자속성2 varchar(5) , -- 가변길이의 문자타입, varchar(5) -> "유재석" -> [유][재][석] x x 남은 칸은 삭제.
    문자속성3 text , 문자속성4 longtext , -- 4G 대용량
    논리속성 bool -- true 또는 false , tinyint 취급 , 1 또는 0 취급
);
select * from test3;

create table comment(
	content varchar(255) , -- 내용 최대 255글자
    writer varchar(20) -- 작성자 최대 20글자
);
select * from comment;
-- [*] 대기명단 기록하는 테이블설계
create table waiting(
	count tinyint unsigned , -- 인원수 (최대 정수 255)
...
create table test4(
    ...
    속성명2 smallint unique, 			-- unique : 해당 속성값들은 중복 불가능
	...
	constraint primary key(속성명4)	-- primary key : pk(식별/기본)키, 식별가능한 고유한 값 가진 키
									-- 중복없음unique + 빈칸없음 not null , 학번 , 사번, 주민등록번호, 제품번호 등등auto_increment
									-- vs constraint primary key ( PK 속성명 )
);
create table test5(
	속성명1 bigint ,
    constraint foreign key( 속성명1 ) references test4( 속성명4 ) on delete set null
    -- foreign key : fk(참조/외래)키, 다른 테이블의 기본키(pk)참조하는 키
    -- constraint foreign key( FK속성명 ) references test4( pk필드명 ) 
    -- 중복가능 , 빈칸가능 ,
	-- pk(1) : fk(N) 관계에서 수강신청학번, 휴가신청학번, 구매제품번호 등등
    -- on update/delete restrict : fk와 pk 참조 관계일때 pk 삭제/수정 불가능 관계 < 자식이 있으면 부모 삭제 불가능 >, 기본값
									-- cascade : pk가 삭제/수정 될 때 fk 필드의 레코드도 같이 삭제 관계, < 게시물삭제되면 댓글도 같이 삭제 >
                                    -- set null : pk가 삭제/수정 될 때 fk는 ull 변경 관계, <게시물삭제되면 댓글 참조는 null>
);
select * from test4;
select * from test5;
...
create table member(
    ...
    mname varchar(30) , -- 회원닉네임 , 문자타입(~30)
...
select * from product;
-- ER 다이어그램 : 여러 객체들 간의 관계 시각적 , ERD 다이어그램 : 데이터베이스 ㅔㅌ이블 간의 관계 시각적
-- 워크벤치 메뉴 상단 -> [ Database ] -> [ Reverse engineer ] -> [ Database 선택 ]