...
select book_id as 도서목록 from orders group by book_id;

-- [문제 4] books 테이블에서 저자(author)별로 그룹화하여 저자 목록만 조회하세요.
select author as 저자 from books group by author;
...
-- 출력: customer, 총구매금액
select customer, sum(price * order_qty) as 총구매금액 from orders, books group by customer;

-- [문제 13] orders와 books를 활용해 장르(genre)별 총 매출을 구하세요.
-- 출력: genre, 총매출
select genre, sum(price * order_qty) as 총매출 from orders, books group by genre;
...
-- 출력: customer, 총구매금액
-- HAVING 총구매금액 >= 100000
select customer, sum(price * order_qty) as 총구매금액 from orders, books group by customer having 총구매금액 >= 100000;
