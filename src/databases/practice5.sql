...
select 제품명, 카테고리명 from product pd join pcategory pc on pd.카테고리번호_fk = pc.카테고리번호_pk;

-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. (조회결과: 2개 레코드)
select 제품명, 제품가격 from product pd join pcategory pc on pd.카테고리번호_fk = pc.카테고리번호_pk where pc.카테고리번호_pk = 1;

-- [문제 3] 모든 제품의 제품명과 등록된 재고수량을 함께 조회하세요. (조회결과: 8개 레코드)
select 제품명, 재고수량 from product pd join stock st on pd.제품번호_pk = st.제품번호_fk;

-- [문제 4] '그램 15인치' 제품의 모든 재고등록날짜와 재고수량을 조회하세요. (조회결과: 2개 레코드)
select 제품명, 재고등록날짜, 재고수량 from stock st join product pd on pd.제품번호_pk = st.제품번호_fk where 제품명 = '그램 15인치';

-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
select * from product pd join pcategory pc on pd.카테고리번호_fk = pc.카테고리번호_pk join stock st on pd.제품번호_pk = st.제품번호_fk;

-- [문제 6] 모든 카테고리의 카테고리명과 해당 카테고리에 속한 제품명을 조회하세요. 만약 카테고리에 속한 제품이 없더라도 카테고리명은 모두 표시되도록 하세요. (조회결과: 7개 레코드)
select 카테고리명, 제품명 from pcategory pc join product pd on pc.카테고리번호_pk = pd.카테고리번호_fk;

-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. (조회결과: 1개 레코드)
select 제품명 from product pd left join stock st on pd.제품번호_pk = st.제품번호_fk where st.제품번호_fk is null;

-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
select sum(재고수량) 총재고수량, 카테고리명 from product pd join stock st on pd.제품번호_pk = st.제품번호_fk join pcategory pc on pd.카테고리번호_fk = pc.카테고리번호_pk group by pc.카테고리번호_pk;  

-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드)
select 제품명, sum(재고수량) 총재고수량 from product pd join stock st on pd.제품번호_pk = st.제품번호_fk group by pd.제품번호_pk order by 총재고수량 desc;